...
ALTER TABLE artist ADD CONSTRAINT fk_label FOREIGN KEY (labelid) REFERENCES recordLabel (labelid);

CREATE TABLE Album (ArtistId int not null, AlbumId int not null,Name varchar(50) not null,Year int not null);
...
ALTER TABLE album ADD CONSTRAINT fk_artist FOREIGN KEY (artistid) REFERENCES artist (artistid);
	
CREATE TABLE Song (AlbumId int not null,SongId int not null,Name varchar(50) not null,Duration real not null);
...
ALTER TABLE song ADD CONSTRAINT fk_album FOREIGN KEY (albumid) REFERENCES album (albumid);
...
--7. Which year (or years) had recorded the most albums?

--8. Total duration of all songs recorded by each artist in descending order.
